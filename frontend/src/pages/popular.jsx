...
export default function TopRated() {
  const [popularMovies, setPopularMovies] = useState([]) // 人気の映画リスト用のstate
  ...
  useEffect(() => {
    const fetchTopRatedMovies = async () => {
      try {
        const response = await fetch('/api/getPopularMovies') 
        if (!response.ok) {
          throw new Error('Network response was not ok')
        }
        const data = await response.json() // レスポンスをJSONに変換
        setPopularMovies(data.results) // データをStateに保存
      } catch (error) {
